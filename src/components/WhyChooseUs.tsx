...
const DEFAULT_SECTION_TITLE = "Your Trusted Daufuskie Island Transportation";
const DEFAULT_INTRO =
...
const DEFAULT_BADGE = "Island Rental Carts Experience";
...
export function WhyChooseUs({
  ...
  const benefits =
    cmsBenefits && cmsBenefits.length > 0 ? cmsBenefits : defaultBenefits;
  const heading = sectionTitle?.trim() || DEFAULT_SECTION_TITLE;
  const introText = intro?.trim() || DEFAULT_INTRO;
          ...
          <div className="flex flex-col lg:flex-row items-center gap-20">
            ...
            <div className="w-full lg:w-1/2 relative group">
              ...
              <div className="relative z-10 rounded-[3rem] overflow-hidden shadow-2xl aspect-4/3 transform transition-transform duration-500 group-hover:-translate-x-2 group-hover:-translate-y-2 border-4 border-brand-700">
                ...
                />
                <div className="absolute inset-0 bg-linear-to-t from-brand-800/60 to-transparent opacity-60" />
              </div>
            </div>
              ...
              <ul className="space-y-6">
                {benefits.map((item) => (
                  <li key={item} className="flex items-center gap-5 group/item">
                    <div className="shrink-0 w-10 h-10 rounded-full bg-lime/20 flex items-center justify-center group-hover/item:bg-lime transition-colors duration-300 border border-lime/30">
